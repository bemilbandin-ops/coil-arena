...
class PresentationGameScene extends ComfortGameScene {
  create(): void {
    ...
    // The old game changed camera zoom continuously. On a native-size canvas that
    // makes both tiny food and screen-space HUD elements resample every frame.
    // Keep one stable logical 1280x720 world view and let only the native fit scale
    // map it to the physical canvas.
...
const config: Phaser.Types.Core.GameConfig = {
  ...
  scale: {
    ...
    // cameras then map the existing 1280x720 coordinates into a centered 16:9
    // viewport; there is no final CSS stretch of a 1280x720 render buffer.
